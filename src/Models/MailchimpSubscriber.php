...
class MailchimpSubscriber extends DataObject implements PermissionProvider
{
    ...
    public function getCMSFields()
    {
        ...
        if($dc && $this->SubscribedWebId) {
            $subscriber_profile_link = sprintf('https://%s.admin.mailchimp.com/lists/members/view?id=%s', $dc, $this->SubscribedWebId);
        } else {
     ...
     */
    public function updateMergeFields(array $meta): void {
        if($meta === []) {
            return;
        }
    ...
    public static function checkExistsInList(string $list_id, string $email, string $api_key = ''): array|false {
        ...
        if( $hash = self::getMailchimpSubscribedId($email) ) {
            $result = self::api()->get(
                sprintf('lists/%s/members/%s', $list_id, $hash)
            );
    ...
    public function subscribe(): bool
    {
        try {
            ...
            if($existing === [] || $existing === false) {
                ...
                $operation_path = sprintf('lists/%s/members/%s', $list_id, $existing['id']);
                $params = $this->getSubscribeRecord();
                // Use current status
            ...
            if ($succeeded) {
                ...
                $errors = sprintf('%s|%s|%s', $error_status, $error_title, $error_detail);
            } else {
    ...
    private function getCurrentSubscriberTags(bool $force = false, int $count = 10) : array {
        ...
        $list = [];
        $offset = 0;
        $operation_path = sprintf('lists/%s/members/%s/tags/?count=%d&offset=%d', $list_id, $subscriber_hash, $count, $offset);
    ...
    protected function modifySubscriberTags() : bool {
        ...
        $list_id = $this->MailchimpListId;
        $subscriber_hash = self::getMailchimpSubscribedId($this->Email ?? '');
        $operation_path = sprintf('/lists/%s/members/%s/tags', $list_id, $subscriber_hash);
        ...
        if($error = self::api()->getLastError()) {
            Logger::log(sprintf('FAIL:%s List:%s', $error, $list_id), 'WARNING');
            return false;
        } else {
            return true;
        }
        ...
        try {
            ...
            if ($report_only) {
                ...
                foreach ($subscribers as $subscriber) {
                    Logger::log(sprintf('REPORT_ONLY: would subscribe #%d to list %s', $subscriber->ID, $subscriber->MailchimpListId), 'DEBUG');
                }
